...
const TEST_NODE = namehash.hash("shutanaka.eth");
...
async function main() {
  ...
  l2ethers.provider = new ethers.providers.JsonRpcProvider(
    hre.network.config.l2url
  );
  console.log(2);
  // Deploy L2 resolver and set addr record for test.test
  const l2accounts = await l2ethers.getSigners();
  const OptimismResolver = await l2ethers.getContractFactory(
    "OptimismResolver"
  );
  console.log(3);
